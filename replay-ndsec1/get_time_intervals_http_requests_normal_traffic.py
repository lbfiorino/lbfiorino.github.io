...
for r in http_requests:
    print(r)
    if (r[1]=="GET"):
        s.get(HOST+r[2], allow_redirects=True)
    elif (r[1]=="POST" and r[2]=="/DVWA/login.php"):
        s.post(HOST+r[2], data={"username": "admin", "password": "password"}, allow_redirects=True)
    elif (r[1]=="POST" and r[2]=="/DVWA/security.php"):
        s.post(HOST+r[2], data={"security": "low"}, allow_redirects=True)
